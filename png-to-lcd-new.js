 ...
 */
var floydSteinberg = require('./floyd-steinberg.js');
var pngparse = require('./pngparse.js');
...
function png_to_lcd(filename, dither, callback) {
  ...
  pngparse.parseFile(filename, function(err, img) {
    ...
    //----------------------------------------------------------
	// new filter pixels to monochorme image data
	// modify by: David Wang
	var bg = false;
	for (var i = 0; i < pixelsLen; i += depth) {
		var grayscale = pixels[i] * 0.3 + pixels[i + 1] * 0.59 + pixels[i + 2] * 0.11;

			if (bg) {
				pixels[i] = grayscale > 127 ? 255 : 37;
				pixels[i + 1] = grayscale > 127 ? 255 : 30;
				pixels[i + 2] = grayscale > 127 ? 255 : 51;
				pixels[i + 3] = 255;
			} else {
				pixels[i] = pixels[i + 1] = pixels[i + 2] = grayscale > 127 ? 255 : 0;
						pixels[i + 3] = 255;
			}
		// push to unpacked buffer list
        //unpackedBuffer[i/depth] = grayscale;
	}	
	//----------------------------------------------------------
